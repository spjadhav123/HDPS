...
class LeavingCertificateScreen extends ConsumerStatefulWidget {
  const LeavingCertificateScreen({super.key});

  @override
  ConsumerState<LeavingCertificateScreen> createState() => _LeavingCertificateScreenState();
}

class _LeavingCertificateScreenState extends ConsumerState<LeavingCertificateScreen> {
  String _search = '';
  Student? _selectedStudent;
  final TextEditingController _reasonController = TextEditingController(text: 'Parents Request');
  ...
  Future<void> _generateAndPrintCertificate(Student student) async {
              ...
              children: [
                ...
                pw.Paragraph(
                  text: 'This is to certify that Master/Miss ${student.name}, '
                      'son/daughter of ${student.parent}, was a bonafide student of this school.',
                  style: const pw.TextStyle(fontSize: 16, lineSpacing: 5),
                ),
                pw.SizedBox(height: 16),
                pw.Paragraph(
                  text: 'Student Code / Registration Number: ${student.studentCode}.',
                  style: const pw.TextStyle(fontSize: 16, lineSpacing: 5),
                ),
                pw.SizedBox(height: 16),
                pw.Paragraph(
                  text: 'He/She was studying in class ${student.className} during the academic year ${DateTime.now().year}-${DateTime.now().year + 1}.',
                  style: const pw.TextStyle(fontSize: 16, lineSpacing: 5),
                ),
                pw.SizedBox(height: 16),
                pw.Paragraph(
                  text: 'Reason for leaving the school: ${_reasonController.text}.',
                  style: const pw.TextStyle(fontSize: 16, lineSpacing: 5),
                ),
                pw.SizedBox(height: 16),
                pw.Paragraph(
                  text: 'His/Her conduct and character during the period of study in this school '
                      'were found to be Good.',
                  style: const pw.TextStyle(fontSize: 16, lineSpacing: 5),
                ),
                pw.SizedBox(height: 16),
                pw.Paragraph(
                  text: 'All school dues have been settled. We wish him/her the best in future endeavors.',
                  style: const pw.TextStyle(fontSize: 16, lineSpacing: 5),
                ),
                pw.Spacer(),
                pw.Row(
    ...
    await Printing.layoutPdf(
      ...
      name: 'Leaving_Cert_${student.name.replaceAll(' ', '_')}',
    );
  }
        ...
        child: Column(
          ...
          children: [
            const PageHeader(
              ...
              subtitle: 'Search for a student to generate a school leaving certificate',
            ),
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.output_rounded, size: 64, color: Colors.grey.shade300),
          const SizedBox(height: 16),
          Text(
            'Select a student to view details\nand generate leaving certificate',
        ...
        children: [
          ...
          TextField(
            ...
            decoration: const InputDecoration(
              ...
              hintText: 'e.g. Parents Request, Relocation, etc.',
              border: OutlineInputBorder(),
            ),
          ),
          const SizedBox(height: 24),
